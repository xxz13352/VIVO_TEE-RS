...
export function parseLicenseStatus(value) {
  const status = String(value).trim();
  return ['verified', 'missing', 'expired', 'device_mismatch', 'invalid_signature', 'invalid_format', 'invalid_product', 'invalid_key', 'clock_rollback', 'state_persistence'].includes(status)
    ? status
    : 'unavailable';
}
